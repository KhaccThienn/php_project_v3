...
$start = ($pages - 1) * $limit;
$sqlProd = "SELECT p.* FROM product p JOIN category c ON c.id = p.category_id WHERE p.name LIKE '%$keyword%' AND p.category_id LIKE '%$cate%' AND p.price BETWEEN '$minPrice' AND '$maxPrice' AND c.status = 1";
        ...
        <div class="form-group">
          <div class="list-group">
            <?php foreach ($result as $key => $value) { ?>
              <a href="?category=<?= $value['id'] ?>" class="list-group-item list-group-item-action <?= ($value['id'] == $cate) ? 'active' : '' ?> "><?= $value['name'] ?></a>
            <?php } ?>
          </div>
        </div>
